...
def tokenize_raw_data(raw_data_path, token_data_path, rev_dic_path,
    ...
    for i,item in enumerate(pbar):
        ...
        parser_dict=_truncate_ast_by_code(parser,3,min(len(parser.code_tokens)-1,3+max_code_len),max_ast_size=max_ast_size,renew_pos=True)
        ...
        text_tokens=tokenize_code_str(item['text'], user_words=USER_WORDS, lemmatize=True, lower=True,
                          keep_punc=True, rev_dic=rev_dic, punc_str=''.join(parser.puncs),
                          operators=parser.operators, pos_tag=False)
        ...
        text_tokens=list(filter(lambda x:len(set(x)-puncs)>0, text_tokens)) #filter out some puns except ; : , . ?
